...
ECO_FEATS feature_extractor::extractor(cv::Mat image, cv::Point2f pos, vector<float> scales, const eco_params& params,const cv::Mat& yml_mean, bool useDeepFeature, boost::shared_ptr< Net<float> >& net)
{
...
cv::Mat feature_extractor::sample_patch(const cv::Mat& im, const cv::Point2f& poss, cv::Size2f sample_sz, cv::Size2f output_sz, const eco_params& gparams)
{
	...
	if (df > 1)
	{
		...
		new_im = new_im2;
		int m, n;
		for (size_t i = 0 + os.y, m = 0; i < im.rows && m < new_im.rows; i += df, ++m)
...
ECO_FEATS feature_extractor::get_cnn_layers(vector<cv::Mat> im,const cv::Mat& yml_mean)
{
	...
	for (size_t idx = 0; idx < cnn_features.fparams.output_layer.size(); ++idx)
	{
		...
		for (size_t i = 0; i < shape[0] * shape[1]; i++)                              //  CNN into single channel******
		{
...
void feature_extractor::cnn_feature_normalization(ECO_FEATS& cnn_feat_maps)
{
	for (size_t i = 0; i < cnn_feat_maps.size(); i++)
	{
		...
		for (int k = 0; k < temp.size(); k++) //*** normalization ****
